...
- iOS bundle: `com.shepherdai.app`, iOS 15+, version `1.0.0`

## Supabase provisioning

1. Link the production project and apply migrations in order with `npx supabase db push`.
2. Set confirmed-email Auth, PKCE, site URL `https://shepherdai.app`, and allow these redirects:
   - `https://shepherdai.app/auth/callback`
...
After Vercel production smoke tests and Supabase advisor review pass, point DNS to Vercel. Keep the legacy deployment available but do not change it during the seven-day observation window. Review Vercel and Supabase errors daily. If a release-blocking error appears, restore DNS to the prior target while fixing forward from GitHub.
